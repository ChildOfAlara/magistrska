...
for filename in os.listdir(path):
    ...
    for line in lines:
        ...
        x0 = 0.5
        resitev = scipy.optimize.newton(f_DeltaF, x0, fprime=f_DeltaFprime, args=(), tol=10**100000000000000000000)
        print resitev

        #def konstanta(w):
            #return (0.002304*w*(-AA + 3 + w**(AA - 3)*(AA - 3)/w)/(-w + 1)**2 + 0.004608*w*(AA - w*(AA - 3) + w**(AA - 3) - 4)/(-w + 1)**3 + 0.002304*(AA - w*(AA - 3) + w**(AA - 3) - 4)/(-w + 1)**2)*w/(1 + (AA-2)*0.048 + ((AA-2)*((AA-2)-1)*0.048**2)/2 + 0.048**2*w*(((AA-2)-2)-((AA-2)-1)*w + w**((AA-2)-1))/((1-w)**2)) - Fhexp
        #def konstantaprime(w):
            #return (0.002304*w*(-AA + 3 + w**(AA - 3)*(AA - 3)/w)/(-w + 1)**2 + 0.004608*w*(AA - w*(AA - 3) + w**(AA - 3) - 4)/(-w + 1)**3 + 0.002304*(AA - w*(AA - 3) + w**(AA - 3) - 4)/(-w + 1)**2)
        #resitev = scipy.optimize.newton(konstanta, x0, fprime=konstantaprime, args=(), tol=1000000000000000000000000000000000000000)

        #Ne izračuna, hkrati ne javi napake (prekompleksno?)
        #solve(DeltaF, w)
        #fprime=DeltaFprime
        #print type(DeltaF)
        def f_test(w):
